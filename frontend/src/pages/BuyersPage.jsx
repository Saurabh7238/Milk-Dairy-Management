...
const emptyBuyer = { name: '', phone: '', address: '', notes: '' };
...
export default function BuyersPage() {
  ...
  const editBuyer = (buyer) => {
    setBuyerForm({ name: buyer.name, phone: buyer.phone, address: buyer.address || '', notes: buyer.notes || '' });
      ...
      <div className="rounded-3xl border border-white/40 bg-white/60 p-5 shadow-lg">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead><tr className="bg-slate-100 text-left"><th className="p-3">Name</th><th className="p-3">Phone</th><th className="p-3">Address</th><th className="p-3">Action</th></tr></thead>
            <tbody>{buyers.map((buyer) => <tr key={buyer._id} className="border-b"><td className="p-3">{buyer.name}</td><td className="p-3">{buyer.phone}</td><td className="p-3">{buyer.address || '-'}</td><td className="p-3"><div className="flex gap-2"><button onClick={() => editBuyer(buyer)} className="rounded-lg bg-sky-600 px-3 py-1 text-white">Edit</button><button onClick={() => deleteBuyer(buyer._id)} className="rounded-lg bg-rose-600 px-3 py-1 text-white">Delete</button></div></td></tr>)}</tbody>
          </table>
        </div>
      </div>
